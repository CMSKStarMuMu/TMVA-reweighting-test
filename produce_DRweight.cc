...
//  auto fin = TFile::Open(Form("/eos/cms/store/group/phys_bphys/fiorendi/p5prime/ntuples/after_nominal_selection/%iMC_JPSI_noIP2D_addxcutvariable.root",year));
  ...
  auto fin = TFile::Open(Form("/gwpool/users/dini/p5prime/ntuples/after_nominal_selection/%dMC_JPSI_noIP2D_addxcutvariable.root",year));
  ...
  for (int i=0; i<tin->GetEntries(); ++i) {
    ...
    if (TMath::IsNaN(DRweight)) {
     std::cout<<"IsNaN(DRweight)!! Setting DRweight=0..."<<std::endl;
     DRweight=0;
    }
